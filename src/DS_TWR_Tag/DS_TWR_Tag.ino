...
#define RESPONSE_TIMEOUT 100 // Response에 대해 100ms 동안만 응답을 기다림
#define REPORT_TIMEOUT 100 // Report에 대해 100ms 동안만 응답을 기다림
...
static unsigned long ranging_start_time = 0; // Response 타임아웃 타이머
static unsigned long report_start_time = 0; // Report 타임아웃 타이머
...
void loop(){
  switch (curr_stage) {
    // stage 1: Transmit the Poll message
    case 0:  // Start ranging.
      ...
      ranging_start_time = millis(); // Response 타임아웃 타이머 시작
      curr_stage = 1;
      break;

    // stage 2: Receive the Response message
    case 1:  // Await first response.
      // 수신된 프레임이 있는지 확인함
      if (rx_status = DW3000.receivedFrameSucc()) {
        DW3000.clearSystemStatus();
        if (rx_status == 1) { // 프레임 수신에 성공한 경우
          // 에러 프레임인지/stage 2에 대한 응답 맞는지 확인
          if (DW3000.ds_isErrorFrame()) {
            ...
            Serial.print("[Error] Received frame with incorrect stage: ");
            Serial.println(DW3000.ds_getStage());
            DW3000.ds_sendErrorFrame();
          } else {
            // 정상적인 응답 메시지인 경우 디버그 메시지를 출력함
            if (DEBUG_PRINT){
            ...
            // 수신된 데이터 저장
            saveResp(anchor_all, tag, DW3000.getSenderID(), DW3000.ds_getRespCount(), DW3000.readRXTimestamp());
            // 리시버 다시 활성화
            DW3000.standardRX();
          }
        }
      ...
      // Timeout logic
      // 모든 앵커의 응답을 받았거나 || 지정된 타임아웃이 지났는지 매번 확인
      if (allRespReceived(anchor_all) || (millis() - ranging_start_time > RESPONSE_TIMEOUT)) {
        // Timeout일 경우, 어떤 앵커에서 응답이 오지 않았는지 확인함
        if (!allRespReceived(anchor_all)) {
            Serial.print("[WARNING] Response phase timed out. Received from: ");
            if(anchor_all.AncA.received_resp){
              Serial.print("A ");
              }
            if(anchor_all.AncB.received_resp){
              Serial.print("B ");
            }
            if(anchor_all.AncC.received_resp){
              Serial.print("C ");
            }
            if(anchor_all.AncD.received_resp){
              Serial.print("D ");
            }
            Serial.println();
        }
        ...
        if (DEBUG_PRINT){
          Serial.println("[INFO] Response phase complete. Moving to stage 2.");
        }
        curr_stage = 2;
      }
      break;

    // stage 3: Transmit the Final message
    case 2:  // Response received. Send second ranging.
      ...
      report_start_time = millis(); // Report 타임아웃 타이머 시작
      curr_stage = 3;
      break;

    // stage 4: Receive the Report message
      ...
      if (rx_status = DW3000.receivedFrameSucc()) {
        ...
        if (rx_status == 1) { // If frame reception was successful
          if (DW3000.ds_isErrorFrame()) {
            ...
            saveReport(anchor_all, DW3000.getSenderID(), DW3000.read(0x12, 0x04),  DW3000.read(0x12, 0x08), DW3000.getRawClockOffset());
            DW3000.standardRX();          
          }
        }
        else {
        ...
        if (allReportsReceived(anchor_all) || (millis() - report_start_time > REPORT_TIMEOUT)) {
          if (!allReportsReceived(anchor_all)) {
            Serial.println("[WARNING] Report phase timed out.");
            }
          curr_stage = 4;
          }
          break;
